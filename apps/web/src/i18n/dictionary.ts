  ...
  uz: {
    ...
    legal: {
      ...
      privacy: {
        ...
        retentionBody: "Hisoblar va serverdagi foydalanuvchi yozuvlari bo‘lmagani uchun VoltAI siz haqingizda saqlaydigan yagona ma’lumotlar o‘z qurilmangizda turadi — ilova ma’lumotlarini tozalaguningizcha yoki ilovani o‘chirguningizcha (bunda sozlamalar, avtomobillar, saqlangan sayohatlar va stansiyalar ro‘yxatining oflayn nusxasi o‘chiriladi). Bizning tomonimizda o‘chiradigan shaxsiy ma’lumot yo‘q: reja so‘rovlari biror shaxsga bog‘lab saqlanmaydi, siz tanlagan koordinatalar hech qachon doimiy xotiraga yozilmaydi, keshda esa faqat ilovaga o‘rnatilgan shaharlar ro‘yxatidagi shaharlar orasidagi marshrutlar turadi — bular sizning joylashuvingiz emas, ilovadagi qat’iy belgilangan joylar. Ilova va VoltAI xizmati o‘rtasidagi tarmoq almashinuvi standart shifrlangan ulanish (HTTPS/TLS) orqali amalga oshiriladi; foydalanuvchi hisoblari bo‘lmagani uchun sizib chiqishi mumkin bo‘lgan shaxsiy ma’lumotlar bazasi ham yo‘q.",
  ...
  ru: {
    ...
    legal: {
      ...
      privacy: {
        ...
        retentionBody: "Поскольку нет учётных записей и серверных записей о пользователях, единственные данные, которые VoltAI хранит о вас, находятся на вашем собственном устройстве — пока вы не очистите данные приложения или не удалите его (при этом удаляются предпочтения, автомобили, сохранённые поездки и офлайн-копия списка станций). На нашей стороне удалять нечего: запросы планов не сохраняются в привязке к человеку, выбранные вами координаты никогда не записываются в постоянное хранилище, а кэшируются только маршруты между городами из встроенного в приложение списка — фиксированными точками приложения, а не вашим местоположением. Обмен между приложением и сервисом VoltAI идёт по стандартному шифрованному соединению (HTTPS/TLS); поскольку учётных записей нет, нет и базы персональных данных, которая могла бы утечь.",
